...
public class RemoveAllCardsFromHandTest : MonoBehaviour
{
    ...
    private async void BeginCardTest(CompanionCardTestPlayer inPlayer)
    {
        ...
        AddToLog("--- Removing all cards from player hand...");
        await CardsTool.singleton.RemoveAllCardsFromPlayerHand(inPlayer.gameboardId, cardHandId);

        List<CardDefinition> cardsLeftInPlayerHand = CardsTool.singleton.GetAllCardsInCardHand(cardHandId);

        AddToLog("--- Cards remaining in test hand should be 0. Actual count is: " + cardsLeftInPlayerHand.Count);
    }
